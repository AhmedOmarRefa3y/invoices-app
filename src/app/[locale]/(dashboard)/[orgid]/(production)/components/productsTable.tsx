...
const ItemsTable = ({
  ...
  type: "raw" | "product" | "plan" | "planProducts" | "partsProducts" | "initialQuantities";
  items: {
    id: string;
    name: string;
    unit: string;
    ...
    <div className="relative overflow-x-auto text-base">
      <table className="w-full bg-white border border-stone-300 ">
        ...
        <tbody>
          ...
          {items.map((item, i) => (
            <tr key={i}>
              <th className="border border-stone-300">{items.indexOf(item) + 1}</th>
              <td className="w-[55%] px-2 bg font-bold text-base border border-stone-300">
                {item.name}
              </td>
              <td className="w-[10%] text-center font-bold border border-stone-300">{item.unit}</td>
