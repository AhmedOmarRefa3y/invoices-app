...
export const SaveReturnedInvoice = async (InvoiceData: saveREtInvoiceType) => {
  try {
    const { InvoiceItems, customerId, date, invoiceAmount, orgid } = InvoiceData;
    if (!orgid) {
      throw new Error("orgid  is required");
    }
    if (!customerId) {
...
export const DeleteReturnedInvoice = async (Id: string) => {
  try {
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "something went while deleting Returned Invoice ",
      data: null,
    };
  }
};
...
export const UpdateReturnsInvoice = async (InvoiceData: UpdateInvoiceType) => {
  try {
    ...
    // update invoice
    const Invoice = await prismaDb.returnedInvoice.update({
      where: {
        id: Id,
      },
      data: {
    ...
    return {
      status: "ok",
      message: "Returns invoice updated succesfully",
      data: Invoice,
    };
  } catch (error) {
    // console.log(error);
    return {
      status: "error",
      message:
        error instanceof Error ? error.name : "something went while updating Returns invoice ",
      data: null,
    };
  }
};
