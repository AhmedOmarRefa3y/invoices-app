...
const CommandItemSelect = ({
  ...
  return (
    <td
      align="center"
      className="p-0 font-semibold  duration-300 border border-stone-300 border-s-0 border-t-0  "
    >
          ...
          <PopoverContent className="sm:w-[497px] relative rounded-none p-0 border border-stone-400 ">
            <Command className="rounded-none">
              ...
              <div className="flex items-center justify-center">
                <Button
                  variant="default"
                  onClick={() => SetAddProdctModalIsOpen(true)}
                  className="w-full text-xl rounded-none hover:bg-black/80 "
                >
                  {t("add_product")}
                  <PlusCircle className="w-5 h-5 ms-2" />
                </Button>
              </div>
            </Command>
          </PopoverContent>
