...
const useInitaliQuanttiesStore = create<ProdcutionStoreT>()((set, get) => ({
  ...
  AddProduct(product) {
    ...
    const isProductAllreadyThere = InitaliQuanttiesProducts.find((item) => item.id === product.id);
    if (isProductAllreadyThere) {
      ...
      set(() => ({
        InitaliQuanttiesProducts: [...InitaliQuanttiesProducts, product],
      }));
    }
  },
  updateProduct(product) {
    const InitaliQuanttiesProducts = get().InitaliQuanttiesProducts;
    const index = InitaliQuanttiesProducts.findIndex((item) => item.id === product.id);
  ...
  DeleteProduct(id) {
    const InitaliQuanttiesProducts = get().InitaliQuanttiesProducts;
    const filterdITems = InitaliQuanttiesProducts.filter((item) => item.id !== id);
    set(() => ({
